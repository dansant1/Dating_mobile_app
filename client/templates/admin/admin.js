...
function subirFotoProducto (event, template, tiendaId) {
...
function subirFotoBanner (event, template, url) {

    let archivo = document.getElementById("banner");
...
Template.AdminProductos.onCreated(function () {
  ...
  self.autorun(function () {
    self.subscribe('productos', Meteor.user().profile.tiendaId);
  });
});

Template.AdminProductos.helpers({
  productos: function () {
...
Template.AdminProductos.events({
  'submit form': function (e, t) {
    e.preventDefault();
    let datos = {
      nombre: t.find("[name='titulo']").value,
    ...
    if (datos.nombre !== "" && datos.descripcion !== "" && datos.precio !== "") {
      ...
      Meteor.call('agregarProductos', datos, function (err, result) {
        if (err) {
          alert('Hubo un error');
        } else {
          ...
          subirFotoProducto(e, t, result.id);
        }
      });
...
Template.AdministradorPublicidad.onCreated(function () {
  var self = this;
...
Template.AdministradorPublicidad.helpers({
  banners: function () {
    return BannersPub.find({});
  }
});

Template.AdministradorPublicidad.events({
